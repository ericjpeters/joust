...
;*
;       NLIST
;       INCLUDE SHRAMDEF.SRC    DONT FORGET STANDARD DEFINITIONS                ;;We don't need these
;       INCLUDE SHORTEQU.SRC    CMOS                                            ;;Was a shorter EQU file needed because of the BSO assembler's limited symbol table size?
...
;*************************************************************************
;*
        ORG     LavaTrollRam           ;NEW RAM FOR THE LAVA TROLL
...
HIDE2   EQU     $30             ; (ETC.)                                        ;;Fixme was: HIDE2  EQU '0
...
;;ATX1  EQU     $11     ;'WELCOME TO JOUST'                                     ;;Already defined in MESSEQU2.ASM
;;ATX2  EQU     $12     ;'TO FLY,'                                              ;;Already defined in MESSEQU2.ASM
;*                      ;'REPEATEDLY PRESS 'FLAP' BUTTON'
;;ATX3  EQU     $13     ;'TO SURVIVE A JOUST                                    ;;Already defined in MESSEQU2.ASM
;*                      ;'THE HIGHEST LANCE WINS'
;*                      ;'IN A COLLISION'
;;ATX4  EQU     $14     ;'PICK UP THE EGGS'                                     ;;Already defined in MESSEQU2.ASM
;*                      ;'BEFORE THEY HATCH'
;;ATX5  EQU     $15     ;'MEET THY ENEMIES'                                     ;;Already defined in MESSEQU2.ASM
;;ATX6  EQU     $16     ;'BOUNDER (500)'                                        ;;Already defined in MESSEQU2.ASM
;;ATX7  EQU     $17     ;'HUNTER (750)'                                         ;;Already defined in MESSEQU2.ASM
;;ATX8  EQU     $18     ;'SHADOW LORD (1500)'                                   ;;Already defined in MESSEQU2.ASM
;;ATX9  EQU     $19     ;'PRESS 1 PLAYER TO START'                              ;;Already defined in MESSEQU2.ASM
;*                      ;'OR'
;*                      ;'INSERT ADDITIONAL COIN FOR'
;*                      ;'2 PLAYER EXCITMENT'
;;ATX10 EQU     $1A     ;'READY FOR 2 PLAYER'                                   ;;Already defined in MESSEQU2.ASM
;*                      ;'JOUST'
;;ATX11 EQU     $1B     ;'PTERODACTYL BEWARE'                                   ;;Already defined in MESSEQU2.ASM
;;ATX13 EQU     $1D     ;'TEMPORARY SAFETY'                                     ;;Already defined in MESSEQU2.ASM
...
FTLEN                   ;LENGTH OF THIS MINI-TABLE
;*
...
ScoreRamLocation  RMB     2       ;SCORE RAM LOCATION
ScoreScreenLocation  RMB     2       ;SCORE SCREEN LOCATION
LivesRemainingScreenLocation  RMB     2       ;SCORE'S AREAS IMAGES OF A PERSON SITTING DOWN
...
WaveMessage_Row0   RMB     4       ;MSG FOR WAVE 'XX'
WaveMessage_Row1   RMB     4       ;MSG NBR & COLOR & SCREEN ADDRESS
WaveMessage_Row2   RMB     4       ;MSG NBR & COLOR & SCREEN ADDRESS
WaveMessage_Row3   RMB     4       ;MSG NBR & COLOR & SCREEN ADDRESS
WaveMessage_Row4   RMB     4       ;MSG NBR & COLOR & SCREEN ADDRESS
WaveMessage_Row5   RMB     4       ;MSG NBR & COLOR & SCREEN ADDRESS
...
;*
                                    ORG     $0
WaveState_BounderCount                              ;NBR OF BOUNDERS IN THIS WAVE (UPPER NIBBLE)
WaveState_HunterCount               RMB     1       ;NBR OF HUNTERS IN THIS WAVE (LOWER NIBBLE)
WaveState_ShadowLordCount                           ;NBR OF SHADOW LORDS IN THIS WAVE (UPPER NIBBLE)
WaveState_PersuerCount              RMB     1       ;NBR OF PERSUERS IN THIS WAVE (LOWER NIBBLE)
WaveState_PterodactylCount          RMB     1       ;NBR OF PTERODACTYLS IN THIS WAVE
WaveState_Status                    RMB     1       ;STATUS BYTE FOR THIS WAVE
WaveState_Length                    EQU     *

;*
;*      WAVE STATUS BYTE, BIT DEFINITION
;*
WaveState_ImmediatePersue           EQU     $01     ;IF=1, PERSUE WILL BE INSTANTANIOUSLY
WBJSR0                              EQU     $02     ;THESE 3 BITS ARE A JSR TABLE OFFSET
WBJSR1                              EQU     $04     ; (0 = AN NOP, OTHERS ARE 2,4,6,8,10,12,14)
WBJSR2                              EQU     $08
WaveState_Cliff1LeftDestroyed       EQU     $10     ;IF=1, CLIFF1L IS DISABLED (OR DESTROYED)
WaveState_Cliff1RightDestroyed      EQU     $20     ;IF=1, CLIFF1R IS DISABLED (OR DESTROYED)
WaveState_Cliff2Destroyed           EQU     $40     ;IF=1, CLIFF2 IS DISABLED (OR DESTROYED)
WaveState_Cliff4Destroyed           EQU     $80     ;IF=1, CLIFF4 IS DISABLED (OR DESTROYED)
WaveState_Cliff1BothDestroyed       EQU     WaveState_Cliff1LeftDestroyed + WaveState_Cliff1RightDestroyed
WaveState_Cliff1BothAnd2Destroyed   EQU     WaveState_Cliff1BothDestroyed + WaveState_Cliff2Destroyed
WaveState_Cliff1BothAnd4Destroyed   EQU     WaveState_Cliff1BothDestroyed + WaveState_Cliff4Destroyed
WaveState_Cliff2And4Destroyed       EQU     WaveState_Cliff4Destroyed + WaveState_Cliff2Destroyed
WaveState_CliffAllDestroyed         EQU     WaveState_Cliff1BothDestroyed + WaveState_Cliff2Destroyed + WaveState_Cliff4Destroyed
...
VCREDIT                 JMP     SCREDIT         ;DISPLAY CURRENT NUMBER OF CREDITS LEFT
VSNHIGH                 FDB     SNHIGH          ;HIGH SCORE TO DATES SOUND
VHSTD                   JMP     HSTD            ;TEMPORARY GO TO H.S.T.D. PAGE
VDCRED                  JMP     DCREDIT         ;DISPLAY CREDITS
VSIM                    JMP     GAMSIM          ;SIMULATION GAME OR INSTRUCTIONAL PAGE
VDCRE2                  JMP     DCRE2           ;DISPLAY CREDITS (VARIABLE BACKGROUND)
VNULL                   FDB     NULL            ;COLOR RAM OF ALL ZEROS
...
.10S    LDX     PDIST,U                                                         ;;Fixme was: .10S
        ...
        BEQ     .11S
        LEAX    PAMSG,U
.12S    CMPA    ,X+             ;IS THE MESSAGE IN THIS SPOT?                   ;;Fixme was: .12S
        BNE     .12S            ; BR=NO
...
.11S    PCNAP   8                                                               ;;Fixme was: .11S
        ...
        LDA     3,X             ;SLEEP TIME
        STA     PJOYT,U
.19S    LDA     #60/2/3                                                         ;;Fixme was: .19S
        STA     PEGG,U
.20S    LDA     PAMSG+0,U                                                       ;;Fixme was: .20S
        BSR     .30S
        PCNAP   2
        LDA     PAMSG+1,U
        BSR     .30S
...
;*
.30S    BEQ     .40S            ;!!!!!!!!!BECAREFULL!!!!!!!!!!                  ;;Fixme was: .30S
        BLT     *+2+3           ;!!!!!!!!!BECAREFULL!!!!!!!!!!
        JMP     TEXT            ;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
;*                              ;!!!!!!!!!BECAREFULL!!!!!!!!!!
        ANDA    #$7F            ;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        JMP     TEXT35          ;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
.40S    RTS                                                                     ;;Fixme was: .40S
;*
ATMRTS  RTS
...
.10S    LDA     PPOSY+1,U
        ...
        LDD     #.35S
        STD     PJOY,U
.20S    CLRB                                                                    ;;Fixme was: .20S
.30S    CLRA                                                                    ;;Fixme was: .30S
        STD     CURJOY
        RTS
;*
.35S    LDA     PVELY,U
...
GAMOVR  LDA     #$80            ;DISABLE CREDIT DISPLAY PAGE
        ...
        CLR     CurrentWave_PersuerCount          ;NO-ONE WILL BE SMART
        LDX     [PDUMMY]
GOVID   LDX     PLINK,X         ;NEXT PROCESS
      ...
      IF DEBUG <> 0             ;???????????? DEBUG PARAMATER ?????????         ;;Fixme was: IFN DEBUG
        CMPY    #P1DEC
        BLO     .1S
        CMPY    #P7DEC
        BLS     .2S
.1S     SWI                     ;THIS IS NOT A DECISION BLOCK
      ENDIF                     ;???????????? END OF DEBUG ????????????
.2S     LDA     PID,X           ;CARD THIS PROCESS
      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
...
.3S     LDY     DJOY,Y
      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
        ...
        BEQ     .4S
        SWI
.4S     LDA     PID,X   ;REG.X=WORKSPACE REG.A=ID REG.Y=DJOY ADDRESS
      ENDIF                     ;???????????? END OF DEBUG ????????????
        STY     PJOY,X
        BRA     GOVID
;*
...
;*
GOVERM  LDX     #$3090
        LDD     #256*Message_ThyGameIsOver+WHI*$11     ;PUT UP GAME OVER MESSAGE
        JMP     OUTPHR
;*
...
ATTRC2  CLR     CRDCHG          ;RESET CHANGE IN CREDITS
        ...
        BEQ     .10S            ; BR=NO-ONE ENTERED THEIR NAME
...
.41S            LDB $C804
        ...
        BNE     .4S             ; BR=YES, IGNORE REQUEST
.3S     BITB    #$10            ;2 PLAYER GAME?
        BEQ     .2S             ; BR=NO, MUST BE 1 PLAYER GAME
        ADDA    #$99
        DAA
.2S     ADDA    #$99            ;       1 LESS CREDIT
        DAA
        STA     CREDIT          ;NEW CREDITS LEFT
        LDX     #CREDST         ;REMEMBER IN CMOS FOR FUTURE POWER-UP
        JSR     WCMSA
.1S     LDA     #8              ;ASSUME BOOKS SINGLE PLAYER START
        CLR     NumberOfPlayers          ;ASSUME 1 PLAYER GAME
        BITB    #$10            ;2 PLAYER GAME
        BEQ     GAM1P           ; BR=1 PLAYER GAME
        LDA     #9              ;ASSUME BOOKS DUAL PLAYER START
        INC     NumberOfPlayers          ;2 PLAYER GAME
...
;*
        ...
        BLS     .1S             ; BR=NO
        INC     ,S
.1S     LDX     #DYTBL          ;ROM INITILIZATION
        LDY     #DYNADJ         ;RAM AREA
.2S     LDB     ,S              ;START VALUE OFFSET
        ASLB
        LDD     B,X
        STD     ,Y
        LDA     #2
...
.10S    LDY     ,U++            ;WRITE THIS BCKGND BLOCK
...
.20S    SECCR   OPPCOL,$20      ;CREATE OPPONENTS COLISION DETECT ROUTINE
;*
        ...
        BGT     .30S            ; BR=YES
        LDX     #P1DEC
.30S    STX     PDECSN,Y        ;PLAYER 1'S JOYSTICK
        LDA     NumberOfLives          ;NBR OF LIVES FOR THIS PLAYER
        JSR     PLYRUP
        LDA     NumberOfPlayers          ;CREATE PLAYER #2?
        BEQ     NOPLY2          ; BR=NO
        ...
        BGT     .40S            ; BR=YES
        LDX     #P2DEC
.40S    STX     PDECSN,Y        ;PLAYER 2'S JOYSTICK
        LDA     NumberOfLives          ;NBR OF LIVES FOR THIS PLAYER
        JSR     PLYRUP
        PCNAP   8
...
ZapTrigger                  PCNAP       2
                            JSR         JZAP
                            ENDIF
ZapTriggerSkipped           PROCCR      THORSE,$6F
...
BCKYUP  LDB     WCLENY,Y                ;CALC LOWEST POINT
        EORB    #4                                                              ;;Fixme was: !DmaHorizontalWidthFIX
        ADDB    WCY,Y
        JSR     VWR1ALL
        ...
        RTS
;*
BRIDGE  FDB     $1200+LIB*$11,0,$00D3,$1B03!DMAFIX                              ;;Fixme was: !XDMAFIX
BRIDG2  FDB     $1200+LIB*$11,0,$78D3,$1E03!DMAFIX                              ;;Fixme was: !XDMAFIX
;*
     IF DEBUG = 0                                                               ;;Fixme was: IFE DEBUG
;*
...
PTERO   DEC     PPVELX,U        ;TIME TO ZOOM IN ON THE PLAYER?
        ...
        BLS     .48S
        DEC     PFEET,U
.48S    LDD     PPOSX,X         ;SAVE HIS CO-ORDINATES
        STD     PDIST,U
        LDB     PPOSY+1,X
        CMPB    #$D3-1          ;NEVER SEEK HIM BELOW CLIFF5
        BLO     .98S
        LDB     #$D3-1
.98S    SUBB    #5              ;ADJUSTMENT FOR CENTER OF BIRD/PTERODACTYL
        JSR     PATCH4          ;PTERODACTYL BETTER KILLER (AIM LOWER)
;********       STB     PRDIR,U         X=PDIST, Y=PRDIR
.47S    LDB     PRDIR,U         ;GET VICTIMS Y POSITION
...
;********       DEC     PEGG,U          ATTACK SEQUENCE TIME OUT?
        ...
        SUBD    #13
        BPL     .2S             ; BR=NO
        BRA     .3S             ;YES, SELECT NEXT VICTUM
.1S     ADDD    #13
        BMI     .2S             ; BR=NO
.3S     LDD     #PTERO          ;HERE IF ENDING ATTACK SEQUENCE
        STD     PJOY,U
        CLR     PRDIR,U         ;NO LONGER CHASE AFTER PLAYER
        CLRB
        JMP     PTEDIR
;*
.2S     LDA     PIMAGE,U        ;ATTACKE FRAME?
        CMPA    #FLY3-FLY1
        BNE     .20S
        LDA     #10
        STA     PACCX,U         ;STAY ON ATTACK FRAME
.20S    LDA     PPOSY+1,U       ;STAY ON CENTER LINE, (OF THE PLAYER)
        CMPA    PRDIR,U
        BEQ     PTEALE          ;ATTACKING, LEVEL FLIGHT
        BLO     PTEADN          ;ATTACKING, GO DOWN
        LDA     PVELY,U         ;NEED TO GO UP, ALREADY GOING UP?
        BMI     .21S            ; BR=YES, NOT TIME TO GO UP YET
        LDD     PVELY,U
        SUBD    #$0010
        STD     PVELY,U         ;GO UP
.21S    BRA     PTEDIR
...
PTEDIS  TST     PBUMPX,U
        ...
        BRA     .10S
;*
.30S    LDA     #4              ;ASSUME GOING RIGHT
        TST     PVELX,U         ;MAINTAIN SLOWER VELOCITY!
        BPL     .10S            ; BR=GOING RIGHT
.20S    LDA     #-4             ;REALLY GOING LEFT
.10S    STA     PVELX,U         ;SPEED
...
PTEDIR  LDA     PFACE,U         ;FLY IN DIRECTION OF FACING
        ...
        BRA     .10S
;*
.30S    LDA     #4*2            ;ASSUME GOING RIGHT
        TST     PVELX,U         ;MAINTAIN SLOWER VELOCITY!
        BPL     .10S            ; BR=GOING RIGHT
.20S    LDA     #-4*2           ;REALLY GOING LEFT
.10S    STA     PVELX,U         ;SPEED
        STA     PFACE,U         ; & FACING DIRECTION
        LDD     #0
        STD     CURJOY
...
PTEKL2  JSR     CPLYR           ;ERASE THE PTERODACTYL
        ...
        LDA     PJOYT,U
        ANDA    #~$03           ;FACE RIGHT 1/2 WAY THROUGH THE DEATH DANCE     ;;Fixme was: ANDA  #!N$03
        BNE     .10S
        CLR     PFACE,U
.10S    DEC     PJOYT,U         ;TIME OUT?
        BPL     PTEKLP
;*
...
;*
        ...
        LDD     #Message_1000*256+($11*PTC)   ;PTERODACTYLE SCORE & COLOR
        STD     PRDIR,U
        JMP     SCRAIR
;*
;*
...
;********STD    PPICR,U
        ...
        BEQ     .2S             ; BR=NO PLAYER1
        JSR     SELARE
        LDX     TARPL2
        BEQ     .2S             ; BR=NO PLAYER2
        JSR     SELARE
.2S     LDX     #$D3-2          ;FOR BOTTOM CLIF5 AREA
        LDA     AREA3
        BEQ     .3S
...
PTESTF  DEC     PACCX,U         ;DECREMENT FRAME HOLDING TIME
        BGT     .30S            ; BR=STILL THIS FRAME
        LDB     PFRAME,U        ;NEXT FRAME
        SUBB    #2
        BPL     .20S            ; BR=DO NOT RESET SEQUENCE
...
SRCADP
        ...
        BPL     .1S
        ADDB    #6
.1S     LDX     IPTERO          ;CALCULATE PTERODACTYL FRAME OFFSET
        ABX
        STX     PPICH,U
        ...
        LDB     #$D3-1
        STB     PPOSY+1,U
.12S    STB     PCOLY1,U        ;MAINTAIN BOTTOM LINE OF COLISION BOX
...
.48S    LDD     PJOY,U          ;ALREADY IN AUTO OFF ROUTINE?
        SUBD    #PTEOFF
        BHS     .49S            ; BR=YES
...
LAVAT1  JSR     LAVVFY          ;MAKE SURE TARGET STILL EXISTS
        ...
        BEQ     LT1HT           ; BR=YES
      IF DEBUG <> 0                                                             ;;Fixme was: IF DEBUG (Is this needed?)
        BLO     .1S
        SWI                     ;PROBLEMS HERE!!
        LDA     #5*6
        BRA     LT1HT
...
LT1HT   LDB     PPOSY+1,Y
        ...
        BHI     .1S             ;BR=HAND TOO HIGH GO DOWN
...
LT1GRP  LDX     #SNTROL         ;MAKE THE SOUND OF THE LAVA TROLL
        ...
        ; ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
        ; This code USED to read: "STD     PPICH,Y         RESET PICTURE AREA IN CASE OF PREMATURE CLEAR"
        ; However, the original authors added a patch here.   To do so, they removed the STD instruction (above)
        ; and replaced it with a JSR to a patch routine.   The first thing done in that patch routine is to
        ; do the "STD PPICH,Y" instruction, then the additional code is executed.
        ;
        ; This patch will increase the lava troll's "strength" in 30 seconds.  It's not clear if that's a 1-time
        ; increase or if it will continue to increase every 30 seconds.   I'm still learning...
        ;
...
.1S     PCNAP   2
        JSR     LAVVFY          ;MAKE SURE TARGET STILL EXISTS
        BRA     .1S             ;LOOP UNTIL SOMETHING HAPPENS
;*
;************************************
;*
...
.40S    LDX     ,S
        ...
        BRA     .30S
;*
.50S    LDX     #14285/2                ;1/2 SEC DEBOUNCE
.55S        LDA   #WatchdogData
    STA   WatchdogTimer
        BSR     READEM
        SUBD    #$0506
        BEQ     .50S
...
WNRM    CLR     EGGS1           ;RESET NUMBER OF EGGS KILLED BY PLAYER 1
        ...
        CLR     CurrentWave_PersuerCount          ;RESET NUMBER OF ALLOWED SMART ENEMIES

      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
        LDA     NSMART          ;THIS HAD BETTER BE ZERO!!!!!!!!!!
        BEQ     .41S
        SWI
.41S    LDA     NENEMY          ;THIS HAD BETTER BE ZERO!!!!!!!!!!
        BEQ     .42S
        SWI
.42S
      ENDIF                     ;???????????? END OF DEBUG ????????????
        ...
        STA     EMYTIM
        CLR     WaveMessage_Row0,U         ;RESET WAVE N MESSAGE SENT AREA
        CLR     WaveMessage_Row1,U         ;RESET MESSAGE BUFFERS
        CLR     WaveMessage_Row2,U
        CLR     WaveMessage_Row3,U
        CLR     WaveMessage_Row4,U
        CLR     WaveMessage_Row5,U
...
;********       JSR     EraseAllWaveMessages           ERASE ANY END OF WAVE MESSAGES
        PCNAP   1
        JSR     WAVEN           ;PUT UP WAVE NUMBER
        LDD     #WaveSetup_NoOp
        STD     PWPREV,U        ;NO PREVIOUS WAVE TO COMPLETE
        LDX     PWAVE,U         ;GET WAVE TABLE ADDRESS
        LDA     WaveState_Status,X       ;GET OLD CLIFF STATE
        LEAX    WaveState_Length,X
        CMPX    #WTBEND         ;AT END OF TABLE??
        BLO     .1S
        LDX     #WTBRST         ;YES, RESTART AT NEW POINT
.1S     STX     PWAVE,U         ;NEW TABLE ADDRESS
        EORA    WaveState_Status,X       ;ANY CLIFFS CHANGED?
        ANDA    #WaveState_CliffAllDestroyed          ;ALL CLIFFS; WaveState_Cliff1BothDestroyed+WaveState_Cliff2Destroyed+WaveState_Cliff4Destroyed
        BEQ     .4S
        LDB     WaveState_Status,X       ;GET CORRECT STATE OF CLIFF
...
.4S     LDX     PWAVE,U         ;GET WAVE AREA TO WORK FROM
        LDA     WaveState_Status,X       ;EARLY PERSUE?
        BITA    #WaveState_ImmediatePersue
        BEQ     .44S            ; BR=NO
        LDA     WaveState_PersuerCount,X       ;SET PERSUE VARIABLE
        ANDA    #$0F
        STA     CurrentWave_PersuerCount
.44S    LDB     WaveState_Status,X               ;CALL WAVE STARTER ROUTINE
        ANDB    #WBJSR0+WBJSR1+WBJSR2   ;MASK OFF OTHER BITS
        LDX     #WaveSetup_SubroutineJumpTable         ;JSR TABLE
        JSR     [B,X]
WPAUSE  LDA     #180/6          ;WAIT A SECOND
WPAUS2  JSR     WAVDEL
        JSR     WATWAV          ;WAIT TILL END OF WAVE
WPAUS3  JSR     EraseAllWaveMessages           ;ERASE ALL MESSAGES
        LDA     [PWAVE,U]       ;INTRODUCE BOUNDARS
        LSRA
        LSRA
        LSRA
        LSRA
        BEQ     .2S             ;BR=NO BOUNDARS
        LDX     #P4DEC          ;(INTELLIGENCE)
        JSR     WCREATE         ;CREATE THEM
.2S     LDA     [PWAVE,U]       ;INTRODUCE hunters
        ANDA    #$0F
        BEQ     .3S             ;BR=NO hunters
        LDX     #P5DEC          ;(INTELLIGENCE)
        JSR     WCREATE         ;CREATE THEM
.3S     LDX     PWAVE,U
        LDA     WaveState_ShadowLordCount,X         ;INTRODUCE SHADOW LORDS
        LSRA
        LSRA
        LSRA
        LSRA
        BEQ     .4S             ;BR=NO BOUNDARS
...
WBEGIN  LDA     GOVER           ;IS THE GAME OVER?
        ...
        ANDA    #$0F
        STA     CurrentWave_PersuerCount
.5S     LDA     PBAITS,U        ;GET BAITERS TABLE DRIVEN TIME
        BEQ     .6S
        DEC     PBAITS,U
.6S     LDX     #BAISBL
...
.9S     LDD     CBAIT
        ...
        BHI     .11S            ; BR=3 BAITERS ON SCREEN, BUT READY
        SECCR   BAITST,PTEID
...
REWMSG  PULS    D               ;SAVE RETURN ADDRESS
        ...
        LDA     GOVER           ;GAME OVER?
        BEQ     .1S
        LDA     #WaveMessage_Row1          ;REMEMBER TO START AT MESSAGE 1
        STA     PMSGA,U
        LDA     WaveMessage_Row0,U         ;THIS MESSAGE O.K. TO PUT OUT?
        BEQ     .2S             ; BR=NO
        JSR     WAVEN
.2S     PCNAP   1
        LDA     PMSGA,U         ;GET CURRENT MESSAGE TO WRITE
        CMPA    #WaveMessage_Row5          ;LAST MESSAGE WAS THE LAST ONE?
        BHI     .1S             ; BR=YES, RETURN TO CALLER
        LEAY    A,U
        ADDA    #WaveMessage_Row2-WaveMessage_Row1    ;NEXT MESSAGE TO WRITE
        STA     PMSGA,U
        BSR     WMSG2
        BRA     .2S
;*
...
;*
;*      DisplayWaveMessage (Subroutine)
;*      Inputs:
;*          * Reg.U = ???
;*      Outputs:
;*      Side Effects:
;*          * Destroys Reg.A, Reg.D, Reg.B, Reg.X, Reg.Y
;*          * Calls OUTPHR
;*      
;* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
DisplayWaveMessage
                    LEAY            WaveMessage_Row1-4,U                ; LEAY = Load Effective Address into Reg.Y -- Y = U + WaveMessage_Row1 - 4
_dwm1               LEAY            4,Y                                 ; Y = Y + 4
                    TST             ,Y                                  ; 
                    BNE             _dwm1                               ; 
                    LDA             GOVER                               ; $00 = GAME IN PROGRESS, $80 = GAME OVER
                    BEQ             _dwm9                               ; If Reg.A = 0, then RTS
                    LDD             ,X                                  ; 
                    STD             ,Y                                  ; 
                    LDD             2,X                                 ; 
                    STD             2,Y                                 ; 
...
;*      Outputs:
;*      SideEffects:
;*          * Destroys Reg.A, Reg.B, Reg.X, Reg.Y
;*          * Calls OUTPHR, WAVEN2
;*
;* ***** ***** ***** ***** ***** ***** ***** ***** ***** ***** *****
EraseAllWaveMessages
        LDA     WaveMessage_Row0,U         ;WAVE MESSAGE SENT TO SCREEN?
        BEQ     .11S            ; BR=NO
        JSR     WAVEN2          ;ERASE WAVE NUMBER
.11S    LEAY    WaveMessage_Row1,U         ;MESSAGE 1
        BSR     .1S
        LEAY    WaveMessage_Row2,U         ;MESSAGE 2
        BSR     .1S
        LEAY    WaveMessage_Row3,U         ;MESSAGE 3
        BSR     .1S
        LEAY    WaveMessage_Row4,U         ;MESSAGE 4
        BSR     .1S
        LEAY    WaveMessage_Row5,U         ;MESSAGE 5
.1S     LDA     ,Y
        BEQ     .2S
...
WCLFEW  PSHS    D,X
        ...
        BEQ     .2S             ; BR=CREATE
.11S    LDY     5,X             ;HERE TO DESTROY, DESTROY TRANSPORTER AREA
        BEQ     .31S
        LDA     #-1
        STA     ,Y
.31S    LDA     1,X             ;DESTROY LANDING AREA
        BEQ     .12S
        COMA
        ...
        JSR     CLRBIT
;*
.12S    LDX     #SNCLIF         ;CLIFF DESTROYING SOUND
        JSR     VSND
...
.1S     PULS    D,X,PC
;*
.2S     LDY     5,X             ;HERE TO CREATE, ENABLE TRANSPORTER
        BEQ     .20S            ; (AYE, AYE SCOTTY)
        CLR     ,Y
.20S    LDA     1,X             ;GET BIT TO PASS FOR CREATING THE CLIFF
        STA     ,-S             ;A DUMB WAY TO PUSH NBRS, BUT ALLOWED
        LDX     #LNDXD1         ;LANDING AREA IN RAM
        LDY     #LNDXS1         ;LANDING AREA IN ROM
.22S    LDA     ,S              ;GET CHANGING BIT
        COMA
        ANDA    ,X
        STA     ,X
...
WAVEN   LDD     #MSW06*256+WHI*$11      ;PUT UP 'WAVE XX'
        STA     WaveMessage_Row0,U         ;THIS MESSAGE HAS BEEN PUT UP
        BRA     WAVEN3
WAVEN2  LDD     #MSW06*256+$00  ;ERASE 'WAVE XX'
        CLR     WaveMessage_Row0,U         ;THIS MESSAGE HAS BEEN ERASED
WAVEN3  LDX     #$3A60
        JSR     OUTPHR
        LDA     WAVBCD
        BITA    #$F0
...
;*
WCLFTB  FCB     WaveState_Cliff1LeftDestroyed,$01,$03
        FDB     CLIF1L,0
WCLFT2  FCB     WaveState_Cliff1RightDestroyed,$00,$00
        FDB     CLIF1R,0
WCLFT3  FCB     WaveState_Cliff2Destroyed,$02,$04
        FDB     CLIF2,CURTR1
WCLFT4  FCB     WaveState_Cliff4Destroyed,$10,$40
        FDB     CLIF4,0
...
;*      Data: $AB, $CD, $EE, $FF
;*
;*      A = 0-15 bounder count
;*      B = 0-15 hunter count
;*      C = 0-15 shadow lord count
;*      D = 0-15 persuer count
;*      EE = 0-? pterodactyl count
;*      FF = wave status
Wave_General                EQU     $0      ; 0 ($0) = NOP ROUTINE
Wave_DisplayIntroMessage    EQU     $2      ; 1 ($2) = INTRODUCTION MESSAGE
Wave_CooperativeWave        EQU     $4      ; 2 ($4) = CO-OPERATION WAVE SET-UP (Survivor in 1P mode)
Wave_GladiatorWave          EQU     $6      ; 3 ($6) = GLADIATOR SET-UP
Wave_EggWave                EQU     $8      ; 4 ($8) = EGG WAVE SET-UP
Wave_PterodactylWave        EQU     $A      ; 5 ($A) = PTERODACTYL WAVE
;*
        FCB     $40, $01, 0, Wave_General                                                                                   ;WAVE 0 (FOR INITILIZATION)
WAVTBL
        FCB     $30, $01, 0,  Wave_DisplayIntroMessage                                                                      ;WAVE 1  - NEEDS INTRO MESSAGE
        FCB     $40, $01, 0,  Wave_CooperativeWave                                                                          ;WAVE 2  - COOP/TEAM/SURVIVE WAVE
        FCB     $60, $02, 0,  Wave_General                                                                                  ;WAVE 3  
        FCB     $33, $01, 0,  Wave_GladiatorWave                                                                            ;WAVE 4  - GLADIATOR WAVE
        FCB     $60, $01, 0,  Wave_EggWave                                                                                  ;WAVE 5  - EGG WAVE
        FCB     $33, $03, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff2Destroyed                           ;WAVE 6  
        FCB     $24, $02, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd2Destroyed           ;WAVE 7  - COOP/TEAM/SURVIVE WAVE

EJP01                                                                                                                                
        FCB     $06, $01, 1, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd2Destroyed           ;WAVE 8  - PTERODACTYL WAVE
        FCB     $06, $02, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 9  - GLADIATOR WAVE
        FCB     $80, $03, 0, Wave_EggWave                                                                                   ;WAVE 10 - EGG WAVE

        FCB     $35, $03, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 11 
        FCB     $26, $02, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                   ;WAVE 12 - COOP/TEAM/SURVIVE WAVE
        FCB     $07, $03, 1, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                 ;WAVE 13 - PTERODACTYL WAVE
        FCB     $08, $04, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 14 - GLADIATOR WAVE
        FCB     $06, $02, 0, Wave_EggWave                                                                                   ;WAVE 15 - EGG WAVE

        FCB     $05, $1F, 0, Wave_General                                                                                   ;WAVE 16 
        FCB     $05, $1F, 0, Wave_CooperativeWave                                                                           ;WAVE 17 - COOP/TEAM/SURVIVE WAVE
        FCB     $05, $1F, 2, Wave_PterodactylWave                                                                           ;WAVE 18 - PTERODACTYL WAVE
        FCB     $04, $2F, 0, Wave_GladiatorWave                                                                             ;WAVE 19 - GLADIATOR WAVE
        FCB     $06, $03, 0, Wave_EggWave                                                                                   ;WAVE 20 - EGG WAVE
                                                                                                                                     
        FCB     $03, $3F, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 21 
        FCB     $02, $4F, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed               ;WAVE 22 - COOP/TEAM/SURVIVE WAVE
        FCB     $02, $4F, 2, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 23 - PTERODACTYL WAVE
        FCB     $02, $4F, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed             ;WAVE 24 - GLADIATOR WAVE
        FCB     $06, $04, 0, Wave_EggWave                                                                                   ;WAVE 25 - EGG WAVE
                                                                                                                                     
        FCB     $03, $5F, 0, Wave_General + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                         ;WAVE 26 
        FCB     $03, $5F, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                 ;WAVE 27 - COOP/TEAM/SURVIVE WAVE
        FCB     $02, $4F, 2, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                 ;WAVE 28 - PTERODACTYL WAVE
        FCB     $03, $5F, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 29 - GLADIATOR WAVE
        FCB     $06, $03, 0, Wave_EggWave                                                                                   ;WAVE 30 - EGG WAVE

        FCB     $04, $4F, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 31 
        FCB     $02, $6F, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 32 - COOP/TEAM/SURVIVE WAVE
        FCB     $02, $4F, 2, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff2Destroyed                   ;WAVE 33 - PTERODACTYL WAVE
        FCB     $02, $6F, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff2Destroyed                     ;WAVE 34 - GLADIATOR WAVE
        FCB     $08, $05, 0, Wave_EggWave                                                                                   ;WAVE 35 - EGG WAVE
                                                                                                                                   
        FCB     $02, $6F, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                           ;WAVE 36 
        FCB     $00, $8F, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                   ;WAVE 37 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $6F, 2, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff2And4Destroyed               ;WAVE 38 - PTERODACTYL WAVE
        FCB     $00, $8F, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff2And4Destroyed                 ;WAVE 39 - GLADIATOR WAVE
        FCB     $08, $05, 0, Wave_EggWave                                                                                   ;WAVE 40 - EGG WAVE
                                                                                                                                   
        FCB     $03, $7F, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 41 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 42 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 43 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 44 - GLADIATOR WAVE
        FCB     $08, $06, 0, Wave_EggWave                                                                                   ;WAVE 45 - EGG WAVE
                                                                                                                                   
        FCB     $03, $7F, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 46 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 47 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue                                               ;WAVE 48 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                     ;WAVE 49 - GLADIATOR WAVE
        FCB     $08, $06, 0, Wave_EggWave                                                                                   ;WAVE 50 - EGG WAVE
                                                                                                                                   
        FCB     $03, $7F, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 51 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 52 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 53 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 54 - GLADIATOR WAVE
        FCB     $08, $0F, 0, Wave_EggWave                                                                                   ;WAVE 55 - EGG WAVE
                                                                                                                                   
        FCB     $03, $7F, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 56 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 57 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue                                               ;WAVE 58 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                     ;WAVE 59 - GLADIATOR WAVE
        FCB     $00, $63, 0, Wave_EggWave                                                                                   ;WAVE 60 - EGG WAVE
                                                                                                                                   
        FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 61 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 62 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 63 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 64 - GLADIATOR WAVE
        FCB     $00, $64, 0, Wave_EggWave                                                                                   ;WAVE 65 - EGG WAVE
                                                                                                                                   
        FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 66 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 67 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue                                               ;WAVE 68 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                     ;WAVE 69 - GLADIATOR WAVE
        FCB     $00, $65, 0, Wave_EggWave                                                                                   ;WAVE 70 - EGG WAVE
                                                                                                                                   
        FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 71 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 72 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 73 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 74 - GLADIATOR WAVE
        FCB     $00, $6F, 0, Wave_EggWave                                                                                   ;WAVE 75 - EGG WAVE
                                                                                                                                   
        FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 76 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 77 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue                                               ;WAVE 78 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                     ;WAVE 79 - GLADIATOR WAVE
        FCB     $00, $86, 0, Wave_EggWave                                                                                   ;WAVE 80 - EGG WAVE
                                                                                                                                   
WTBRST  FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue + WaveState_Cliff1BothDestroyed                       ;WAVE 81 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 82 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue + WaveState_Cliff1BothAnd4Destroyed           ;WAVE 83 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_CliffAllDestroyed                   ;WAVE 84 - GLADIATOR WAVE
        FCB     $00, $8F, 0, Wave_EggWave                                                                                   ;WAVE 85 - EGG WAVE
                                                                                                                                   
        FCB     $00, $AF, 0, Wave_General + WaveState_ImmediatePersue                                                       ;WAVE 86 
        FCB     $00, $AF, 0, Wave_CooperativeWave + WaveState_ImmediatePersue                                               ;WAVE 87 - COOP/TEAM/SURVIVE WAVE
        FCB     $00, $7F, 3, Wave_PterodactylWave + WaveState_ImmediatePersue                                               ;WAVE 88 - PTERODACTYL WAVE
        FCB     $00, $AF, 0, Wave_GladiatorWave + WaveState_ImmediatePersue + WaveState_Cliff4Destroyed                     ;WAVE 89 - GLADIATOR WAVE
        FCB     $00, $8F, 0, Wave_EggWave                                                                                   ;WAVE 80 - EGG WAVE
...
;*
MSCOP2  FDB     $10F8
        FCB     $2A!HIDE+HIDE2,$11      ;(                                      ;;Fixme was: !XHIDE
        FCB     $0D!HIDE+HIDE2  ;C                                              ;;Fixme was: !XHIDE
        FCB     $2B!HIDE+HIDE2  ;)                                              ;;Fixme was: !XHIDE
        FCB     $0A!HIDE+HIDE2  ;SPACE                                          ;;Fixme was: !XHIDE
        FCB     $21!HIDE+HIDE2  ;W                                              ;;Fixme was: !XHIDE
        FCB     $13!HIDE+HIDE2  ;I                                              ;;Fixme was: !XHIDE
        FCB     $16!HIDE+HIDE2  ;L                                              ;;Fixme was: !XHIDE
        FCB     $16!HIDE+HIDE2  ;L                                              ;;Fixme was: !XHIDE
        FCB     $13!HIDE+HIDE2  ;I                                              ;;Fixme was: !XHIDE
        FCB     $0B!HIDE+HIDE2  ;A                                              ;;Fixme was: !XHIDE
        FCB     $17!HIDE+HIDE2  ;M                                              ;;Fixme was: !XHIDE
        FCB     $1D!HIDE+HIDE2  ;S                                              ;;Fixme was: !XHIDE
        FCB     $0A!HIDE+HIDE2  ;SPACE                                          ;;Fixme was: !XHIDE
        FCB     $0F!HIDE+HIDE2  ;E                                              ;;Fixme was: !XHIDE
        FCB     $16!HIDE+HIDE2  ;L                                              ;;Fixme was: !XHIDE
        FCB     $0F!HIDE+HIDE2  ;E                                              ;;Fixme was: !XHIDE
        FCB     $0D!HIDE+HIDE2  ;C                                              ;;Fixme was: !XHIDE
        FCB     $1E!HIDE+HIDE2  ;T                                              ;;Fixme was: !XHIDE
        FCB     $1C!HIDE+HIDE2  ;R                                              ;;Fixme was: !XHIDE
        FCB     $19!HIDE+HIDE2  ;O                                              ;;Fixme was: !XHIDE
        FCB     $18!HIDE+HIDE2  ;N                                              ;;Fixme was: !XHIDE
        FCB     $13!HIDE+HIDE2  ;I                                              ;;Fixme was: !XHIDE
        FCB     $0D!HIDE+HIDE2  ;C                                              ;;Fixme was: !XHIDE
        FCB     $1D!HIDE+HIDE2  ;S                                              ;;Fixme was: !XHIDE
        FCB     $0A!HIDE+HIDE2  ;SPACE                                          ;;Fixme was: !XHIDE
        ...
        FCB     $18!HIDE+HIDE2  ;N                                              ;;Fixme was: !XHIDE
        FCB     $0D!HIDE+HIDE2  ;C                                              ;;Fixme was: !XHIDE
        FCB     $2E!HIDE+HIDE2  ;.                                              ;;Fixme was: !XHIDE
        FCB     0
     ENDIF
...
;*
WaveSetup_SubroutineJumpTable  FDB     WaveSetup_NoOp                           ; 0 ($0) = NOP ROUTINE
        FDB     WaveSetup_DisplayIntroMessage                                   ; 1 ($2) = INTRODUCTION MESSAGE
        FDB     WaveSetup_CooperativeWave                                       ; 2 ($4) = CO-OPERATION WAVE SET-UP
        FDB     WaveSetup_GladiatorWave                                         ; 3 ($6) = GLADIATOR SET-UP
        FDB     WaveSetup_EggWave                                               ; 4 ($8) = EGG WAVE SET-UP
        FDB     WaveSetup_PterodactylWave                                       ; 5 ($A) = PTERODACTYL WAVE

;*
WaveSetup_NoOp  RTS                     ;NOTHING TO DO

;*
WaveSetup_DisplayIntroMessage  LDX     #INTRO1
        JSR     DisplayWaveMessage
        LDA     #90/6
        JSR     WAVDEL
        LDX     #INTRO2
        JSR     DisplayWaveMessage
...
;*
WaveSetup_PterodactylWave  EQU     *               ;PTERODACTYL WAVE
        LDX     #PTER1
        JSR     DisplayWaveMessage
        LDA     #180/6
        JSR     WAVDEL
        ...
        LDA     WaveState_PterodactylCount,X         ; GET NBR OF PTERODCTYLS FOR THIS WAVE
...
;*
WaveSetup_CooperativeWave   LDA     SPLY2+6         ;THIS MESSAGE IS FOR TWO PLAYERS ONLY
...
WSUSCR  LDX     #SURV3          ;ASSUME NO POINTS
        ...
        BRA     ENDTS
;*
.1S     LDA     SPLY2+6         ;STILL HAVE SOME LIVES LEFT?
...
;*
WaveSetup_GladiatorWave   LDA     SPLY1+6         ;THIS MESSAGE IS FOR TWO PLAYERS ONLY
...
WGLSCR
        ...
        BPL     .2S             ; BR=PLAYER 2 GOT THE BOUNTY
        LDX     #GLAD5          ;PLAYER 1 BONUS POINTS MESSAGE
        LDA     PLYG2
        BPL     .3S             ;BR= PLAYER 1 GOT THE BOUNTY
        LDX     #GLAD4          ;NO ONE GOT THE BOUNTY
        BRA     .3S
.2S     LDX     #GLAD6          ;PLAYER 2 BONUS POINTS MESSAGE
.3S     JSR     DisplayWaveMessage
...
;*      START OF EGG WAVE
;*
WaveSetup_EggWave  LDX     #EGG1           ;EGG WAVE
        JSR     DisplayWaveMessage
        LDA     #180/6
        JSR     WAVDEL
        JSR     WATWAV          ;WAIT TILL TRUE END OF WAVE
        JSR     EraseAllWaveMessages           ;ERASE AL MESSAGES
;*
        LDX     PWAVE,U         ;FIND INTELLIGENCE
        LDY     #P6DEC          ;ASSUME THE BEST
        LDA     WaveState_BounderCount,X
        BNE     .1S
        LDA     WaveState_ShadowLordCount,X         ;IT IS THE LORD HIMSELF
        BRA     .2S
.1S     LDY     #P5DEC          ;HUNTER?
        BITA    #$0F
        BNE     .3S             ; BR=YES
...
.3S     ANDA    #$0F
        ...
        BEQ     .35S            ; BR=NO, ALL TIMES ARE BASIED ON 1 PLAYER
        LDA     SPLY1+6         ;2 PLAYERS IN THE GAME
        BEQ     .35S            ; BR=YES, ALL TIMES ARE BASIED ON 1 PLAYER
        LSRB                    ;ON 2 PLAYERS USE 1/2 OF THE TIME
.35S    STB     PEGGTM,U        ;EGG TIMER
        LDA     WaveState_PersuerCount,X       ;NBR TO START PERSUING
        ANDA    #$0F
        STA     CurrentWave_PersuerCount
...
.5S     LDY     #EGLEDG         ;PUT AN EGG ON EVERY LEDGE (RANDOM LEDGE)
        LDX     #EGLTBL
...
.6S     LDX     #EGPTBL         ;POINT TO EGG POSITION TABLE
        JSR     VRAND
        LDB     #69*2
        MUL                     ;REG. A = RANDOM NBR 0-68
.7S     LDB     A,X             ;IS THIS LOCATION FREE?
        BEQ     .8S             ; BR=YES
        INCA                    ;NEXT LOCATION
...
;*      TROMSG
;*
TROMSG  FCB     ($1E+$35)!$9B,($12+$35)!$9B,($13+$35)!$9B,($1D+$35)!$9B         ;;Fixme was: !X$9B
        FCB     ($0A+$35)!$9B,($13+$35)!$9B,($1D+$35)!$9B,($0A+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($14+$35)!$9B,($19+$35)!$9B,($1F+$35)!$9B,($1D+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($1E+$35)!$9B,($2E+$35)!$9B,($00+$35)!$9B                      ;;Fixme was: !X$9B
        FCB     ($0E+$35)!$9B,($0F+$35)!$9B,($1D+$35)!$9B,($13+$35)!$9B         ;;Fixme was: !X$9B
        FCB     ($11+$35)!$9B,($18+$35)!$9B,($0F+$35)!$9B,($0E+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0A+$35)!$9B,($0C+$35)!$9B,($23+$35)!$9B,($0A+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($21+$35)!$9B,($13+$35)!$9B,($16+$35)!$9B,($16+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($13+$35)!$9B,($0B+$35)!$9B,($17+$35)!$9B,($1D+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0A+$35)!$9B,($0F+$35)!$9B,($16+$35)!$9B,($0F+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0D+$35)!$9B,($1E+$35)!$9B,($1C+$35)!$9B,($19+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($18+$35)!$9B,($13+$35)!$9B,($0D+$35)!$9B,($1D+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0A+$35)!$9B,($13+$35)!$9B,($18+$35)!$9B,($0D+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($2E+$35)!$9B,($00+$35)!$9B                                    ;;Fixme was: !X$9B
        FCB     ($2A+$35)!$9B,($0D+$35)!$9B,($2B+$35)!$9B                       ;;Fixme was: !X$9B
        FCB     ($0A+$35)!$9B,($01+$35)!$9B,($09+$35)!$9B                      ;;Fixme was: !X$9B
        FCB     ($08+$35)!$9B,($02+$35)!$9B,($0A+$35)!$9B,($21+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($13+$35)!$9B,($16+$35)!$9B,($16+$35)!$9B,($13+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0B+$35)!$9B,($17+$35)!$9B,($1D+$35)!$9B,($0A+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0F+$35)!$9B,($16+$35)!$9B,($0F+$35)!$9B,($0D+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($1E+$35)!$9B,($1C+$35)!$9B,($19+$35)!$9B,($18+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($13+$35)!$9B,($0D+$35)!$9B,($1D+$35)!$9B,($0A+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($13+$35)!$9B,($18+$35)!$9B,($0D+$35)!$9B,($2E+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($00+$35)!$9B                                                  ;;Fixme was: !X$9B
        FCB     ($0B+$35)!$9B,($16+$35)!$9B,($16+$35)!$9B,($0A+$35)!$9B         ;;Fixme was: !X$9B
        FCB     ($1C+$35)!$9B,($13+$35)!$9B,($11+$35)!$9B,($12+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($1E+$35)!$9B,($1D+$35)!$9B,($0A+$35)!$9B,($1C+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($0F+$35)!$9B,($1D+$35)!$9B,($0F+$35)!$9B,($1C+$35)!$9B        ;;Fixme was: !X$9B
        FCB     ($20+$35)!$9B,($0F+$35)!$9B,($0E+$35)!$9B,$2E                  ;;Fixme was: ($20+$35)!X$9B,($0F+$35)!X$9B,($0E+$35)!X$9B,!W($80+$35!X$9B) (Was getting range overflow error, so I just put in "$2E")
;*
...
DE3HRD  STD     PDECSN,Y
        ...
        BNE     .1S             ; BR=YOU CAN GET MORE EGGS
        TFR     Y,X
...
PLYEGG  EQU     *               ;COLISION DETECT WITH THE EGG
      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
        ...
        BEQ     .12S
        SWI
.12S    LDA     PID,X           ;THIS BETTER BE AN EGG
        CMPA    #$80+EGGID
        BEQ     .13S
        SWI
.13S
...
EGGDB1  LDB     ,Y              ;GET NUMBER OF TIME AN EGG WAS HIT
      ...
      IF DEBUG = 0      ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFE DEBUG
        BHS     EGGSMN
      ELSE
        BLO     .1S
...
;*
        ...
        JSR     ScoreHundreds          ; (IN PLAYERS WORKSPACE)
;*
.1S     DEC     NRIDER          ;1 LESS ENEMY RIDER
      IF DEBUG          ;???????????? DEBUG PARAMATER ?????????
        BPL     .52S
...
.20S    LDX     PLINK,X         ;NEXT PROCESS
        BEQ     .100S           ; BR=END SEARCH FOR OVERLAPPING SCORES
...
.32S    SUBD    #16
        BGE     .20S            ; BR=NO, NOT EVEN CLOSE
.33S    LDA     PJOYT,X         ;A FASTTER EGG  DISPLAY TIME?
        CMPA    #16/2
        BLO     .34S            ; BR=NO ONLY A FEW TICKS LEFT
...
.34S    ASLA
        ...
        BLO     .20S            ; BR=NO, CURRENT DELAY IS THE LONGEST
        STA     PLAVT,U
        BRA     .20S
;*
.100S   LDA     #SCRID          ;EGG SCORES I.D.
        STA     PID,U
...
.101S   PCNAP   1
        ...
        BEQ     .111S           ; BR=NO
        LDX     #.112S          ;DISPLAY THE SCORE IN THE AIR
        LDD     #CATID*256+$FF
        JSR     VCUPROC
        LDD     #Message_500*256+EGGCOL*$11  ;GET SCORE VALUE & COLOR
        STD     PRDIR,Y
        LDD     PPOSX,U
        STD     PPOSX,Y
        LDD     PPOSY,U
...
;*
ATTEGG  LDX     #.20S           ;ATTRACT MODE EGG SCORE CYCLE
        LDD     #EGGID*256+$FF
        JSR     VCUPROC
        LDD     PPOSX,U
        STD     PPOSX,Y
...
.1S
      ENDIF                     ;???????????? END OF DEBUG ????????????
        ...
        BRA     .3S
;*
.2S     LDD     DSMART,X        ;SELECT PROPER JOYSTICK ROUTINE
.3S     STD     PJOY,U
...
LINET   LDA     NSMART          ;GET NUMBER OF SMART ENEMIES
        CMPA    CurrentWave_PersuerCount          ;BELOW MINIMUM INTELLIGENCE?
        ...
        CMPA    #LAVID
        BEQ     LNTUP
.1S     LDA     #AOFFL1         ;ASSUME NEAR TRACKING LINE 1
        LDB     PPOSY+1,U
        CMPB    #AOFFL1+AOFFUD
...
B2UNDR  DEC     PLAVT,U
        ...
        CMPA    #LAVID
        BEQ     B2DN1A
.1S     JSR     SELPLY          ;SELECT TARGETED PLAYER
...
;*                              ;IF HERE, LET HIM LOURE ME INTO LAVA
.3S     LDB     PDIST+1,U       ;AB2VE TRACKING LINE
        CMPB    PPOSY+1,U
        LBLO    B2LEVA          ; BR=YES
B2FAST  LDD     #B2LEV2
        STD     PJOY,U
...
SHLEP1  DEC     PJOYT,U
        ...
        LDA     PVELY,U         ;ALREADY GOING UP?
        BPL     SHFAST          ; BR=NO
.2S     CMPB    PDIST+1,U       ;ABOVE TRACKING LINE
        BLS     SHLEPA          ; BR=YES
SHFAST  LDD     #SHLEP2
        STD     PJOY,U
        LDB     #$01            ;FLAP WINGS
...
.1S     LDA     PVELX,U         ;WHICH DIRECTION GOING?
        BEQ     SHDIRA
        BPL     SHDILR
        PSHS    B
        LDD     PVELY,U         ;SEEKING DESTINATION CLIFF
...
;*
        ...
        BLO     SPN3PL
SPN2PL  LDX     TARPL2
SPN3PL  ANDCC   #~$04           ;CLEAR ZERO BIT FOR .NE. BEING TRUE             ;;Fixme was: ANDCC  #!N$04
        RTS
;*
...
.2S     LDX     PFRAME-2,U
        ...
        BNE     .21S            ;and offset out debris images on the
        LDX     #$F145          ;larger and edge located cliffs
        BRA     .22S
.21S    CMPX    $0004           ;$0000 and $0004 are CLIF1L and CLIF2 vectors,
        BNE     .23S            ;respectively
        LDX     #$3151
        BRA     .22S
.23S    LDX     4,X             ;Get destination
.22S    STX     PFRAME,U        ;Adjust destination for centering on cliffs
        BSR     CLIFER
        STY     PFRAME+3,U      ;we must maintain our why
...
;*      (PFRAME,U)
CLIFER  LDX     PFRAME,U
.2S     LDA     ,Y+     ;lo nybble is color, hi nybble is length
        BEQ     .1S     ;length & color equ 0, end of image
        ANDA    #$0F
        ...
        LSRA
        LSRA
        BNE     .3S     ;if length equ 0, end of line
        LDX     PFRAME,U        ;next line is always 1 down
        LEAX    1,X
        STX     PFRAME,U
        BRA     .2S
.3S     BEQ     .2S     ;  (test is on top for minimal memory)
        CMPA    #1      ;last byte is always one pixel to enhance debris image
        BNE     .31S    ;while all others are bytes for minimal memory
...
LOCCLR  LDB     WCLENY,Y                ;CALC LOWEST POINT
        EORB    #4                                                              ;;Fixme was: EORB  #!DmaHorizontalWidthFIX
        ADDB    WCY,Y
        ...
        EORB    #4              ;the falling debris                             ;;Fixme was: EORB  #!DmaHorizontalWidthFIX
        ADDB    #10+1
        EORB    #4                                                              ;;Fixme was: EORB  #!DmaHorizontalWidthFIX
        STD     WCLEN,X
        RTS
...
STPLY2  LDD     TARPLY          ;IS THIS SPOT EMPTY?
        ...
        STA     TARTM1
        RTS
.1S     STU     TARPL2          ;USING SECOND AREA
        LDA     TARTIM          ;GIVE CURRENT TIME DELAY TIME
        STA     TARTM2
        RTS
;*
...
.12S
      ENDIF                     ;???????????? END OF DEBUG ????????????
        ...
        LDA     TARTM2
        STA     TARTM1
.1S     LDD     #0              ;ASSUME OTHER POINTER
        STD     TARPL2
;*
;*                              CHECK FOR AN END OF GAME
;*
.2S     LDX     PDECSN,U
        LDX     ScoreRamLocation,X        ;GET PLAYER SCORE/LIVES AREA
      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
...
.22S
      ENDIF                     ;???????????? END OF DEBUG ????????????
        ...
        BNE     .5S
.4S     LDA     GOVER           ;ATTRACT MODE?
        BPL     .5S             ; BR=YES, GAME IS ALREADY OVER
        ...
        PULS    U
.5S     RTS
.6S      LDX    #GAMOV1         ;GAME OVER FOR PLAYER 1 OR 2
         LDD    #PLYID*256+$FF
         JSR    VCUPROC
...
.1S     LDX     PDECSN,U
        ...
        JMP     VSUCIDE
;*
.10S    LDA     DGAMO,X         ;PUT UP PLAYER 1/2 GAME OVER
        LDX     #$296C
        JSR     OUTPHR
...
.21S
      ENDIF                     ;???????????? END OF DEBUG ????????????
.25S    LDU     PLINK,U         ;FIND TWO COLISIONAL OSTRICHES
...
.42S
      ENDIF                     ;???????????? END OF DEBUG ????????????
.45S    LDX     PLINK,X
        BEQ     .20S            ;BR= END OF PAIRS OF COLISION
        LDA     PID,X
        BPL     .45S
        ANDA    PID,U
        ...
        BEQ     .45S
        BSR     HITEM
        BCC     .40S            ;THERE IS SOMETHING TO COLISION WITH
        BRA     .20S
;*
...
PLYCOL  PCNAP   1               ;COLISION DETECT ROUTINE
        ...
        BEQ     .10S
        DEC     TARTM1
.10S    LDA     TARTM2          ;DECREMENT TARGETED PLAYERS TIME
        BEQ     .15S
        DEC     TARTM2
.15S
      IF DEBUG <> 0     ;???????????? DEBUG PARAMATER ?????????                 ;;Fixme was: IFN DEBUG
...
OSTHIT  LDX     COLOBJ          ;GET OTHER COLISIONED PARTY
        ...
        ANDA    PID,X
        BITA    #$01
        BNE     .100S           ; BR=PTE/PLY/EMY COLISION
        BITA    #$02
...
;**PTE  SUBB    #5      15-4    ;FUDGE FROM LANTZ TO CENTER OF PTERO-MOUTH
        ...
        BPL     .10S
        NEGB
.10S    CMPB    #2              ;WITHIN 5 PIXELS?
.14S    BHI     OSTBO           ; BR=NO, CANNOT KILL PTERODACTYL
        ...
        BPL     OSTBO           ; BR=NO, CANNOT KILL PTERODACTYL
.11S    JMP     OSTPYP          ;BOUNCE KILLER AWAY FROM PTERODACTYL
;*
.12S    LDA     PFACE,U         ;PTERODACTYL ON RIGHT, PLAYER ON LEFT
        BPL     .11S            ; BR=YES, KILLED PTERODACTYL
OSTBO   LDB     PLANTZ,U
        SEX
        STD     ,--S
        ...
        SUBD    PPOSY,U
        BEQ     .1S             ; BR=BOTH ON SAME LEVEL
        BMI     OSTXT3
        JMP     OSTPYP          ; BR=REG.Y ON TOP
;*
.1S     LDX     #SNPTHD         ;PLAYERS COLIDE
        JSR     VSND
        LDX     COLOBJ
        BRA     OSTXTT
;*
...
.2S     LDD     PPOSX,X
        ...
        BPL     .3S             ; BR=BIRD IS ON THE RIGHT, SO BUMP HIM RIGHT
        LDA     PVELX,X         ;THE BIRD MOVES LEFT NOW
        BLE     .21S
        SUBA    #2              ;SLOW DOWN A BIT
        NEGA
        STA     PVELX,X
.21S    LDA     PVELX,U         ;BUMP BIRD
        NEGA
        ASLA
        STA     PBUMPX,X
        LDA     #$FF
        ...
        RTS
;*
.3S     LDA     PVELX,X         ;THE BIRD MOVES RIGHT NOW
        BGE     .31S
        NEGA
        SUBA    #2              ;SLOW DOWN A BIT
        STA     PVELX,X
.31S    LDA     PVELX,U         ;BUMP BIRD
        NEGA
        ASLA
        STA     PBUMPX,X
        CLR     PFACE,X         ;MAKE X FACE RIGHT
...
GENDMA  PSHS    CC
        ...
        STD     DmaDestinationOfCharacter
        LDD     ,X++
        EORA    #$04                                                            ;;Fixme was: EORA  #!DmaVerticalHeightFIX
        EORB    #$04                                                            ;;Fixme was: EORB  #!DmaVerticalHeightFIX
        STD     DmaHorizontalWidth
        STX     DmaSourceOfCharacter
        TFR     Y,D
        STB     DmaConstantSubstitution
        STA     DMA
        PULS    CC,PC
...
;*
.30S    CMPA    #$8A+7          ;MIDDLE LEVEL?
        BHI     .40S            ; BR=NO
        LEAY    FTLEN,Y         ; NEXT ENTRY
        LDA     STTR2           ;TRANSPORTER DISABLED?
        BNE     .35S            ; BR=YES, NEXT PERSON
        BSR     TRBOND          ;CHECK IF THIS TRANSPORTER IS AVAILABLE
.35S    LDA     STTR3           ;TRANSPORTER DISABLED?
        BNE     CRENPR          ; BR=YES, NEXT PERSON
        LEAY    FTLEN,Y         ; NEXT ENTRY
        BSR     TRBOND          ;CHECK IF THIS TRANSPORTER IS AVAILABLE
        BRA     CRENPR          ;NEXT PERSON
;*
.40S    CMPA    #$A3+7          ;LOWER LEVEL?
        BLO     CRENPR          ; BR=NO
...
TRBOND
        ...
        BHI     .30S            ; BR=YES
        BRA     .20S            ;FAIL THIS TRANSPORTER
;*
.10S    CMPX    FXMIN,Y         ;IS OBJECT FAR AWAY ENOUGH TO LEFT?
        BHI     .20S            ; BR=NO
        CMPX    FXMAX,Y         ;IS OBJECT FAR AWAY ENOUGH TO RIGHT?
        BHI     .30S            ; BR=YES
.20S    INC     [FLEVEL,Y]      ;NO, FAIL TRANSPORTER
.30S    RTS
;*
...
G1DEC   FDB     G1JOY,G1JOY,SPLY1,$39D9,SCOPL1,PLYR1,TREPL1,CREP1 ;PLAYER 1
        FDB     DEATH1,EGGS1,STPLY1,ScoreHundreds,PLYG1,PLYTIM
...
G2DEC   FDB     G2JOY,G2JOY,SPLY2,$60D9,SCOPL2,PLYR2,TREPL2,CREP2 ;PLAYER 2
        FDB     DEATH2,EGGS2,STPLY2,ScoreHundreds,PLYG2,PLYTIM
...
P3DEC   FDB     AUTOFF,AUTOFF,0000,$0000,SCOPL2,PLYR3,TREPL3,EMYDIE
        FDB     DEATH3,0,STENMY,ScoreTens,0,EMYTIM
...
P6DEC   FDB     LINET,SHADOW,0000,$0000,SCOPL2,PLYR5,TREPL3,EMYDIE
        FDB     DEATH3,0,STENMY,ScoreHundreds,0,EMYTIM
        FDB     SNELWU,SNELWD,SNEMSK,SNEMS2,SNERU1,SNERU2,SNEFAL,0,SNECRE
        FCB     WHI*$11,$15,MSGAMO
P7DEC   FDB     LINET,PTERO,0000,$0000,SCOPL2,0,TREPL3,EMYDIE
        FDB     DEATH4,0,STENMY,ScoreHundreds,0,EMYTIM
...
.10S    LDX     PLINK,X         ;FOR OTHER FLYING OBJECTS
        BEQ     .40S            ; BR=END OF LIST
        LDA     PID,X           ;PROCESS ACTIVE ON SCREEN?
        BPL     .10S            ; BR=NO
        CMPA    #$80+EGGID      ;AN EGG?
        BNE     .30S            ; BR=NO
        LDD     PDIST,X         ;CALLING A BIRD?
        BEQ     .10S            ; BR=NO, SO IGNORE HIM
.30S    JSR     SELARE          ;CHECK IF AREA IS CLEAR
        BRA     .10S
;*
.40S    LDD     CURTR1  ;CALCULATE FOR A FREE TRANSPORTER
        STD     STTR1
        LDD     CURTR3
        STD     STTR3
...
.50S    LDA     AREA2           ;MIDDLE AREA EMPYT?
        BNE     .60S            ; BR=NO
...
.60S    LDA     AREA1           ;TOP AREA EMPYT?
        BNE     .70S            ; BR=NO
        LDA     STTR1           ;TOP TRANSPORTER NOT IN USE?
        BEQ     GOTR1           ; BR=NO, GO USE IT
.70S    LDX     #TRPLY          ;TRANSPORTER SAFTEY AREA
...
GOTTR   INC     [TCURUSE,X]     ;CURRENTLY THIS TRANSPORTER IN USE
        ...
        BRA     .3S
.2S     INC     LESERV  ;ALLOW ENEMIES NEXT NUMBERED CARD TO USE TRANSPORTER
.3S     JSR     VSND
        LDA     #30
        STA     PFRAME,U
        LDY     PDECSN,U
...
TREFF2  PCNAP   1               ;EFFECTS TIME
        ...
        RORB
        STB     WCX,X
        LDA     #(18/2)!4               ;PROPER X LENGTH FOR BOX ERASE                  ;;Fixme was: LDA  #18/2!X(!DmaHorizontalWidthFIX) (Should be $0D - $8C29)
        STA     WCLENX,X
        DEC     PFRAME,U
        LBNE    TREFF
;*
...
;********       BNE     .40S             BR=NO, NO SWITCHES WILL BE USED
;********.10S   JSR     [DJOY,X]        GET 1ST JOYSTICK POSITION
...
;********       BEQ     .140S            BR=YES, IGNORE THESE SWITCHES
;********       CLR     PADGRA+0,U      NO, REMEMBER NO SWITCHES ON
;********       TSTA                    JOYSTICK SWITCHES ON?
;********       BNE     .51S            BR=PLAYER/ENEMY WANTS TO MOVE
;********.140S  CMPB    PADGRA+1,U      FLAP SWITCH IN LAST POSITION?
;********       BEQ     .141S            BR=YES, IGNORE THIS SWITCH
;********       CLR     PADGRA+1,U      NO, REMEMBER NO SWITCHES ON
;********       TSTB                    JOYSTICK SWITCHES ON?
        BNE     .51S            ;BR=PLAYER/ENEMY WANTS TO FLAP
;********.141S
...
.48S    LDA     WCDMA,X
        ORA     #$10
        STD     WCDMA,X
.49S    LEAX    10,X            ;NEXT DMA BUFFER
        PULS    B,PC
;*
.51S    LDX     PDECSN,U        ;ABORTED EARLY, KILL SOUND
        LDX     DSNTREF,X
        BEQ     .50S            ; BR=NO SOUND
        JSR     VSND
.50S    JSR     CPLYR
...
STFALL  LDA     PFRAME,U        ;A SKIDDING FALL OFF A CLIFF?
        ...
        LDX     DSNFAL,X
        JSR     VSND
.1S     CLR     PPOSY+2,U       ;RESET FRACTIONAL DISTANCES
        CLR     PPOSX+2,U
        CLR     PBUMPY,U        ;RESET BUMP Y REGISTER
        LDA     #10
...
ENDAD1  EQU     *       ;CURRENT END ADDRESS
    ...
    IF (* < (GameRamEnd + 1))
        WARNING "\a The module is smaller than the current address space.  [ GameRam: $\{GameRam} -> $\{GameRamEnd} => max $\{(GameRamEnd - GameRam) + 1} bytes allowed vs. actual of $\{* - GameRam} bytes used ($\{(GameRamEnd - *) + 1} bytes unused) ]"
    ELSEIF (* = (GameRamEnd + 1))
        MESSAGE "\a The module is precisely sized to the current address space.  [ GameRam: $\{GameRam} -> $\{GameRamEnd} => $\{GameRamEnd - GameRam + 1} bytes]"
    ELSE
        ERROR "\a The module is too large to fit in the current address space.  [ GameRam: $\{GameRam} -> $\{GameRamEnd} => $\{(GameRamEnd - GameRam) + 1} bytes expected vs. actual of $\{(* - GameRam) + 1} bytes ($\{* - GameRamEnd} bytes too large) ]"
    ENDIF
...
PATCHS  EQU     *       ;START ADDRESS OF PATCHES
        ...
        ;
        ; I'm going to leave this for now, and ATT.SRC still ends at $D6EF as of 2025-07-16
        ;
        IF *-$D6EF <= 0                                                         ;;Fixme was: IFLE  *-$D6EF
...
PATC11  CLR     PLYG1           ;RESET 3,000 POINT TRIGGER
        CLR     PLYG2
        JMP     EraseAllWaveMessages           ;ERASE ANY END OF WAVE MESSAGES(OLD INSTRUCTION)
;*
...
PATCH9  TST     PCHASE,U        ;NON-BAITERS IGNORE THIS
        ...
        BNE     .10S            ;BUT SATURATE IT AT 1
        INC     PPVELX,U
.10S    DEC     PEGG,U          ;ATTACK SEQUENCE TIME OUT?
        RTS
;*
...
.10S    STA     PPVELX,U        ;(RESTORE OLD INSTRUCTION)
        RTS
;*
...
;*                              VERSION 4 SOFTWARE
;*
PATCH6  BLO     .10S            ;BR=TRACK ON LINE 2 (RESTORE OLD 2 INSTRUCTIONS)
        LDA     #AOFFL3-2       ;ASSUME NEAR TRACKING LINE 3
        RTS
;*
.10S    TST     PCHASE,U        ;AGAIN, THIS IS ONLY FOR BAITERS
        BEQ     .30S
        LDD     PPOSX,U         ;TRACKING LINE 2, TO THE RIGHT OF CLIF3U?
        SUBD    #(260+7)
        BGT     .20S            ; BR=YES, GO TO LOWER TRACKING LINE
        SUBD    #(200-28-7)-(260+7)     ;TO THE LEFT OF CLIF3U?
        BGT     .30S            ; BR=NO, USE NORMAL UPPER CLIF3U LINE
.20S    LDA     #$88            ;NEW LINE TO TRACK FOR LOWER CLIFFS
        RTS
;*
.30S    LDA     #AOFFL2-2       ;ASSUME NEAR TRACKING LINE 2
        RTS
;*
...
.10S    STB     PRDIR,U         ;X=PDIST, Y=PRDIR (RESTORE OLD INSTRUCTION)
        RTS
;*
...
PATCH1  STD     PPICH,Y         ;OLD INSTRUCTION
        ...
        ; EJP -- Cannot understand why precisely what this is doing.
        ;
        ; Okay, this is just ensuring that the patches don't overstep into the next 
        ; block of source, which starts at $D800.
        ;
        ; I'm leaving this for now, and encourage it moving forward - it's a meaningful check.
        ;
        IF *-$D800 > 0                                                          ;;Fixme was: IFGT  *-$D800
...
SELARE  LDA     PPOSY+1,X
        ...
        RTS
;*
.1S     CMPA    #$51            ;MIDDLE AREA2 FREE?
        BLO     .2S             ; BR=YES
        INC     AREA2           ;HERE IF NO!
        RTS
;*
.2S     INC     AREA1           ;MUST BE IN TOP AREA1
        RTS
;*
...
;??????
        ...
        CMPB    #7!4            ;GUYS LENGTH IS ASSUMED TO BE 7 PIXELS TALL     ;;Fixme was: CMPB  #7!X(!DmaHorizontalWidthFIX)
...
CLIPER  LDA     WCLENY,X        ;FIND BOTTOM LINE OF OBJECT
        EORA    #4                                                              ;;Fixme was: EORA  #!DmaHorizontalWidthFIX
        ADDA    WCY,X
        ...
        LDD     #$0000!DMAFIX   ;A 1 BYTE TRANSFER                              ;;Fixme was: LDD  #$0000!XDMAFIX
        STD     WCLEN,X
CLPRTS  RTS
;*
CLPIT   NEGA
...
;??????
        ...
        STD     PADGRA,U
.22S    JMP     ADGFLR          ;NOW, THE GUY IS IN THE LAVA
;*
...
;********       LDA     PID,X
;********       CMPA    #LAVID+1
;********       BEQ     .12S             BR=NO, A PROBLEM EXISTS
;********       SWI
;********      .12S
;********            ENDIF                      ???????????? END OF DEBUG ??????
;??????
...
ADLFRE  LDA     PID,U           ;IS THE PERSON ALIVE
        ...
        LDX     PDECSN,U
        JSR     ScoreTens
.33S    LDX     PLINK,U         ;KILL LAVA TROLL GRAB TASK
        LDA     PID,X           ;ANOTHER PROCESS CAN SNEAK IN,
        CMPA    #LAVID+1        ;SO, CHECK TO MAKE SURE THIS IS THE TROLL
        BNE     .11S
        LDD     #LT2DIE
        STD     PPC,X
.11S    LDX     PPREV           ;DROP THE HAND
;********            IFN DEBUG          ???????????? DEBUG PARAMATER ?????????
;********       LDA     PID,X           DEBUG
;********       CMPA    #LAVID
;********       BEQ     .2S
...
;??????
        ...
        LDA     #1
        STA     PNAP,X
.22S    LDD     #ADDGRA         ;BACK TO NORMAL GRAVITY
        STD     PADGRA,U
...
.1S     INC     LAVNBR          ;1 MORE LAVA TROLL
...
STATE   MACRO   WAIT,CALL,MINUS,ZERO,PLUS,FLYVEL
        FCB     WAIT                                                            ;;Fixme was: FCB  WAIT,!HCALL,!WCALL,MINUS-*,ZERO-*,PLUS-*,FLYVEL
        FDB     CALL
        FCB     (MINUS-*)+3,(ZERO-*)+3,(PLUS-*)+3,FLYVEL                        ;;The label for the macro should technically start on this line
        ENDM                                                                    ;; so to get the correct values we compensate by adding 3
...
RUNFRM  STB     PFRAME,U
        ...
        BRA     .2S
;*
.3S     LDX     DSNRU2,X        ;SECOND FOOT RUNNING SOUND
.2S     JSR     VSND
        LDB     PFRAME,U
.1S     ASLB                    ;X2
...
;*      SAVES REG.X
;*
ScoreHundreds  LDY     ScoreRamLocation,X        ;GET SCORE AREA
        BEQ     ScoreRoutineExit
        ADDA    2,Y
        BRA     ScoreCommon
;*
...
;*      SAVES REG.X
;*
ScoreTens  LDY     ScoreRamLocation,X        ;GET SCORE AREA
        BEQ     ScoreRoutineExit
        TFR     A,B
        ANDA    #$F0            ;SCORE TENS
        ANDB    #$0F            ;       & HUNDREDS
...
SCRLEV  LDD     1,Y             ;GET 100'S OF THOUSANDS TO 100'S.
        ...
        BLO     ScoreExtraPlayer          ;AWARD THE EXTRA PLAYER
        ...
        DAA
        STA     7,Y
ScoreExtraPlayer  PSHS    X,Y             ;SAVE DECISION BLOCK
...
.1S     PCNAP   2
        ...
        BNE     .100S           ; BR=MILLIONS TO SCORE
        LEAX    6,X
        LDB     [,X]            ;100,000 & 10,000 CHECK
        BEQ     .2S             ; BR=CHECK HUNDREDS DIGIT
        ANDB    #$F0
        BNE     .100S           ; BR=100,000
        LEAX    6,X
        BRA     .100S           ;HERE IF 10,000
;*
.2S     LEAX    6+6,X
        LDB     [,X]            ;1,000 & 100 CHECK
        BEQ     .3S             ;BR=NO DIGITS HERE
        ANDB    #$F0
        BNE     .100S
...
;*
;*      SOUND TABLE
;*       PIRORITY,SOUND,LENGTH (IF M.S.BIT SET ON SOUND, SOUND,LENGTH)          ;;LOTs of Fixme edits here:
;*              SOUND !N$00 DOES NOT DISTURBE THE SOUND, BUT EXTENDS TIMER      ;; !N = ~
;*              SOUND !N$FF TURNS OFF (KILLS) THE SOUND                         ;; !. = &
;*              SOUND !N$xx (RANGE 01-3E) SENDS THE CORRESPONDING SOUND         ;; !+ = |
;*
SNCRED  FCB     200,~$0A&$7F,40         ;CREDIT CHANGE SOUND                    ;;Fixme was: FCB  200,!N$0A!.$7F,40
SNGS    FCB     200,~$1B&$7F,1          ;GAME START                             ;;Fixme was: FCB  200,!N$1B!.$7F,1
SNHI2   FCB     190,~$1C&$7F,6          ;HIGH SCORE TO DATE END SOUND           ;;Fixme was: FCB  190,!N$1C!.$7F,6
SNHIGH  FCB     190                     ;HIGH SCORE TO DATE                     ;;Fixme was: FCB  190
        FCB         ~$11|$80,60                                                 ;;Fixme was: FCB  !N$11!+$80,60
        FCB         ~$10|$80,20                                                 ;;Fixme was: FCB  !N$10!+$80,20
        FCB         ~$10|$80,30                                                 ;;Fixme was: FCB  !N$10!+$80,30
        FCB         ~$0F|$80,2*60                                               ;;Fixme was: FCB  !N$0F!+$80,2*60
        FCB         ~$10|$80,60                                                 ;;Fixme was: FCB  !N$10!+$80,60
        FCB         ~$10|$80,100                                                ;;Fixme was: FCB  !N$10!+$80,100
        FCB         ~$0F|$80,4*60                                               ;;Fixme was: FCB  !N$0F!+$80,4*60
        FCB         ~$1A|$80,20                                                 ;;Fixme was: FCB  !N$1A!+$80,20
        FCB         ~$1A|$80,100                                                ;;Fixme was: FCB  !N$1A!+$80,100
        FCB         ~$10|$80,60                                                 ;;Fixme was: FCB  !N$10!+$80,60
        FCB         ~$0F|$80,4*60                                               ;;Fixme was: FCB  !N$0F!+$80,4*60
        FCB         ~$1A|$80,30                                                 ;;Fixme was: FCB  !N$1A!+$80,30
        FCB         ~$18|$80,4*60                                               ;;Fixme was: FCB  !N$18!+$80,4*60
;*
        FCB         ~$10|$80,80                                                 ;;Fixme was: FCB      !N$10!+$80,80
        FCB         ~$10|$80,60                                                 ;;Fixme was: FCB      !N$10!+$80,60
        FCB         ~$10|$80,20                                                 ;;Fixme was: FCB      !N$10!+$80,20
        FCB         ~$0F|$80,4*60                                               ;;Fixme was: FCB      !N$0F!+$80,4*60
        FCB         ~$10|$80,20                                                 ;;Fixme was: FCB      !N$10!+$80,20
        ...
        FCB         ~$10|$80,80                                                 ;;Fixme was: FCB      !N$10!+$80,80
        FCB         ~$0F|$80,2*60                                               ;;Fixme was: FCB      !N$0F!+$80,2*60
        FCB         ~$0F&$7F,4*60                                               ;;Fixme was: FCB      !N$0F!.$7F,4*60
SNREPL  FCB     100,~$0B&$7F,90         ;EXTRA MAN                              ;;Fixme was: FCB  100,!N$0B!.$7F,90
SNCLIF  FCB     067,~$19&$7F,90         ;CLIFF DESTROYER                        ;;Fixme was: FCB  067,!N$19!.$7F,90
SNPTED  FCB     066,~$16|$80,15,~$16|$80,15     ;PTERODACTYL DYING SOUND        ;;Fixme was: FCB  066,!N$16!+$80,15,!N$16!+$80,15
        FCB         ~$16|$80,07,~$16|$80,7                                    ;;Fixme was: FCB      !N$16!+$80,07,!N$16!+$80,7
        FCB         ~$16&$7F,90                                                 ;;Fixme was: FCB      !N$16!.$7F,90
SNPTEI  FCB     065,~$24|$80,30,~$25&$7F,30     ;PTERODACTYL INTRODUCTION SCREAM ;;Fixme was: FCB  065,!N$24!+$80,30,!N$25!.$7F,30
SNPTE   FCB     065,~$24&$7F,120                ;PTERODACTYL SCREAM             ;;Fixme was: FCB  065,!N$24!.$7F,120
SNBOUN  FCB     050,~$1C&$7F,60         ;COLLECT BOUNTY                         ;;Fixme was: FCB  050,!N$1C!.$7F,60
SNTROL  FCB     050,~$09&$7F,30         ;CAPTURED BY LAVA TROLL SOUND           ;;Fixme was: FCB  050,!N$09!.$7F,30
SNEGG   FCB     045,~$03&$7F,30         ;PLAYER HITS EGG SOUND                  ;;Fixme was: FCB  045,!N$03!.$7F,30
SNEGGH  FCB     045,~$02&$7F,30         ;EGG HATCHING SOUND                     ;;Fixme was: FCB  045,!N$02!.$7F,30
SNMOUN  FCB     045,~$0C&$7F,30         ;ENEMY MOUNTING THE BIRD                ;;Fixme was: FCB  045,!N$0C!.$7F,30
SNCTHD  FCB     010,~$06&$7F,10         ;CLIFF THUD                             ;;Fixme was: FCB  010,!N$06!.$7F,10
;*
SNECRE  FCB     040,~$07|$80,90,$00,1   ;ENEMY RE-CREATED (TRANSPORTER)         ;;Fixme was: FCB  040,!N$07!+$80,90,$00,1
SNEDIE  FCB     040,~$16&$7F,20         ;ENEMY DIES                             ;;Fixme was: FCB  040,!N$16!.$7F,20
SNELAV  FCB     040,~$0D&$7F,30         ;ENEMY IN LAVA                          ;;Fixme was: FCB  040,!N$0D!.$7F,30
SNETHD  FCB     009,~$08|$80,30,$00,1   ;ENEMIES THUD                           ;;Fixme was: FCB  009,!N$08!+$80,30,$00,1
SNELWU  FCB     006,~$21&$7F,60         ;ENEMIES WING UP SOUND                  ;;Fixme was: FCB  006,!N$21!.$7F,60
SNELWD  FCB     006,~$20&$7F,60         ;ENEMIES WING DOWN SOUND                ;;Fixme was: FCB  006,!N$20!.$7F,60
SNERU1  FCB     006,~$22&$7F,60         ;ENEMY RUNS                             ;;Fixme was: FCB  006,!N$22!.$7F,60
SNERU2  FCB     006,~$23&$7F,60         ;ENEMY RUNS                             ;;Fixme was: FCB  006,!N$23!.$7F,60
SNEMSK  FCB     006,~$26&$7F,60         ;ENEMIES SKIDDING SOUND                 ;;Fixme was: FCB  006,!N$26!.$7F,60
SNEMS2  FCB     006,~$27&$7F,30         ;END OF ENEMIES SKIDDING SOUND          ;;Fixme was: FCB  006,!N$27!.$7F,30
SNEFAL  FCB     006,~$FF&$7F,1          ;ENEMY STARTING TO FALL (SKID STOP)     ;;Fixme was: FCB  006,!N$FF!.$7F,1
;*
SNPDIE  FCB     080,~$16&$7F,20         ;PLAYER DIES                            ;;Fixme was: FCB  080,!N$16!.$7F,20
SNPCR1  FCB     070,~$12|$80,30         ;PLAYER 1 RE-CREATED (TRANSPORTER)      ;;Fixme was: FCB  070,!N$12!+$80,30
        FCB         ~$14|$80,255        ;PLAYER FADING IN  (TRANSPORTER)        ;;Fixme was: FCB      !N$14!+$80,255
        FCB         ~$00&$7F,165                                                ;;Fixme was: FCB      !N$00!.$7F,165
SNPCR2  FCB     070,~$12|$80,30+13      ;PLAYER 2 RE-CREATED (TRANSPORTER)      ;;Fixme was: FCB  070,!N$12!+$80,30+13
        FCB         ~$15|$80,255        ;PLAYER FADING IN  (TRANSPORTER)        ;;Fixme was: FCB      !N$15!+$80,255
        FCB         ~$00&$7F,165-13                                             ;;Fixme was: FCB      !N$00!.$7F,165-13
SNPTREF FCB     070,~$FF&$7F,1          ;PLAYER ABORTED FADING IN  (TRANSPORTER) ;;Fixme was: FCB  070,!N$FF!.$7F,1
SNPLAV  FCB     060,~$0D&$7F,30         ;PLAYER IN LAVA                         ;;Fixme was: FCB  060,!N$0D!.$7F,30
SNPTHD  FCB     020,~$08|$80,30,$00,1   ;AT LEAST 1 PERSON THUD'ED              ;;Fixme was: FCB  020,!N$08!+$80,30,$00,1
SNPLWU  FCB     010,~$21&$7F,90         ;PLAYERS WING UP SOUND                  ;;Fixme was: FCB  010,!N$21!.$7F,90
SNPLWD  FCB     010,~$20&$7F,90         ;PLAYERS WING DOWN SOUND                ;;Fixme was: FCB  010,!N$20!.$7F,90
SNPRU1  FCB     010,~$22&$7F,70         ;PLAYER RUNS                            ;;Fixme was: FCB  010,!N$22!.$7F,70
SNPRU2  FCB     010,~$23&$7F,70         ;PLAYER RUNS                            ;;Fixme was: FCB  010,!N$23!.$7F,70
SNPLSK  FCB     010,~$26&$7F,70         ;PLAYERS SKIDDING SOUND                 ;;Fixme was: FCB  010,!N$26!.$7F,70
SNPLS2  FCB     010,~$27&$7F,40         ;END OF PLAYERS SKIDDING SOUND          ;;Fixme was: FCB  010,!N$27!.$7F,40
SNPFAL  FCB     010,~$FF&$7F,60         ;PLAYER STARTING TO FALL (SKID STOP)    ;;Fixme was: FCB  010,!N$FF!.$7F,60
;*
