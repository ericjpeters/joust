...
RERR6   LDA     #RAM>>8                                                         ;;Fixme was: LDA  #RAM!>8
        TFR     A,DP
        TFR     CC,A
        COMA
...
;*      RESULTING SUM.  IF SUM=0 THEN PART IS NOT STUFFED.
       ...
       IF DEBUG = 0             ;FOR ROM VERSION                                ;;Fixme was: IFE DEBUG
...
BARS0   LDD     ,Y++    ;GET A COLOR
    ...
    STA   WatchdogTimer
        ...
        LDD     #0
        LDX     #0
COLR1   STX     XTEMP                                                           ;;Fixme was: COLOR1 (duplicate symbol)
        LEAX    $F00,X
COLOR2  STD     ,--X
        PSHS    A
            LDA   #WatchdogData
...
CRTTSS  LDX     #COLRMT ;TABLE TO STORE                                         ;;Fixme was: CRTT$$ (invalid symbol name)
...
BOOK    LDA     #RAM>>8                                                         ;;Fixme was: LDA  #RAM!>8
        TFR     A,DP
...
.1S     TFR     B,A     ;COPY B INTO A
        COMA
        PSHS    D
        JSR     RCMSD
        TST     ,S+     ;GET THE COMPLEMENTED BYTE BACK (FULLY BLANKED = 00)
        BNE     .2S     ;BRA= FIRST BYTE WAS NOT FULLY BLANKED
...
.2S     LDA     #$99    ;DIGIT BY DIGIT (THERE MUST BE A FASTER WAY!)
        SUBA    ,-Y
        STA     ,Y
        DECB
        BNE     .2S
...
.10S    BRA     CURS    ;WRITE THE CURSOR OUT IN IT'S NEW POSITION
.4S     CMPY    #ATOP   ;IS HE AT THE TOP OF THE ADJUSTMENTS
        BEQ     NOMOVE  ;BRA= YES SO DON'T DO A THING
        BSR     NOCURS  ;ERASE THE CURSOR
...
.3S     PSHS    A
    ...
    STB   WatchdogTimer
        ...
        ADDA    #$99    ;BCD -1
        BRA     CMWRIT
.10S    CMPA    1,X     ;IS IT AT IT'S MIN.
        BEQ     CWRET   ;BRA= YES..
        ADDA    #$01    ;BCD +1
...
PRICOUT PSHS    D,U,X,Y
        LDX     #$0438!DMAFIX                                                           ;;Fixme was: LDX  #$0438!XDMAFIX
        STX     $CA06
        LDX     #$6A48
        STX     $CA04
        CLRB
...
SUB2    LDD     #$040C!DMAFIX                                                           ;;Fixme was: LDD  #$040C!XDMAFIX
        BRA     SUB3
SUB1    LDD     #$0608!DMAFIX                                                           ;;Fixme was: LDD  #$0608!XDMAFIX
SUB3    STD     $CA06
        STX     $CA04
        LDA     XTEMP
        ADDA    #$11
...
CLRTAB  FCB     $AF
        ...
        FCB     $0A
;*
TSTEND  EQU     *                                                                       ;;Fixme was: ENDADR (duplicate symbol)
