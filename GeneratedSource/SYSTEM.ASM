...
;*
;       INCLUDE RAMDEF.SRC      ;DONT FORGET STANDARD DEFINITIONS               ;;We don't need these
...
;*  DATA DIRECTION
        STA     PIA_ControlA
        STA     PIA_ControlB
        STA     PIA_InputA           ;INPUTS
        STA     PIA_InputB           ;INPUTS
        LDB     #%00110100      ;$34
        STB     PIA_ControlA          ;SETUP, WIDGET FOR SWITCH INPUTS
        STB     PIA_ControlB
;*
...
;*
      IF DEBUG2 <> 0                                                            ;;Fixme was: IFN DEBUG2
...
SUCIDE  LDS     #STACK          ;STACK IS IN THE CLEAR
      ...
      IF DEBUG2 = 0                                                             ;;Fixme was: IFE DEBUG2
;*
...
CUPROC  PSHS    D
        LDY     PFREE
      IF DEBUG2 <> 0                                                            ;;Fixme was: IFN DEBUG2
         BNE    CreateProcessRam          ;BR=RAM AVAILABLE
         SWI                    ;HERE IF OUT OF PROCESS BLOCKS
      ENDIF
CreateProcessRam  LDD     PLINK,Y         ;REMOVE NEW LINK FROM..
...
IRQSLM
        ...
        LDD     SWT0
        STD     SWT0+1
        LDA     PIA00            ;READ SWITCHES (3 COINS,H.S. RESET,ADVANCE)
        STA     SWT0+0
...
IRQDMA  LDS     #DMA+8          ;START/RESTART DMA LOOPING
        ...
        INC     >PIA11          ;ENABLE PIA INTERUPTS TO MICRO-PROCESSOR
...
;*
RANDSYS LDA     RANDOM+1        ;RANDOM BIT (IN CARRY)                          ;;Fixme was: RAND
        LSRA
        LSRA
        LSRA
        EORA    RANDOM+1
...
SND     LDA     ,X+             ;GET SOUND PIRORITY
        BMI     .1S             ;SOUND PIRORITYS OF 128 TO 255 ARE ALWAYS SENT
        LDB     GOVER           ;NO SOUNDS 0 TO 127 DURING END OF GAME
        BPL     NOSND
.1S     LDB     STMR            ;SOUND STILL SOUNDING                           ;;Fixme was: 1$
...
LNYBB   EQU     16      ;nybble moving constants left nybble = nybble *16       ;;Fixme was: LN (duplicate symbol)
...
NEXTLN  PCNAP   20              ;Sleep long enough for the cliffs to be in place
        ...
        ANDA    #~$01                                                           ;;Fixme was: ANDA  #!N$01
        STA     DRRUC           ;THIS 1ST BECAUSE OF INTERUPTS USING THIS BIT
        STA     RRUC
.1S     STB     ,X                                                              ;;Fixme was: 1$
        LEAX    $100,X
        CMPX    #$9600          ;MAXIMUM SCREEN ADDRESS?
        BHI     SPECIAL         ; BR=YES
        LDA     ,X              ;is place in where ever
        ANDA    #$EE
        BEQ     .1S             ;BR = we don't have cliff
;*
SP1     BITA    #$F0            ;here we may have a pixel left over
        BNE     .2S             ; but then we must do the right side lava level
        ORA     #CLR3*LNYBB                                                     ;;Fixme was: ORA  #CLR3*LN
        STA     ,X
.2S     LDD     SAFRM2                                                          ;;Fixme was: 2$
...
;*
        ...
        TFR     D,X
        STX     PFRAME+4,U
        LDA     #CLR3*LNYBB                                                     ;;Fixme was: LDA  #CLR3*LN
        STA     ,X
        ...
        STA     0,X
        STA     $100,X
        LDA     #CLR3*LNYBB     ;$30                                            ;;Fixme was: LDA  #CLR3*LN
        STA     -1,X
        STA     1,X
        PCNAP   8
        LDX     PFRAME+4,U
...
.10S    INCA                    ;1 MORE ENCODED LENGTH                          ;;Fixme was: 10$
        ASLB                    ;GET NEXT BIT
        BNE     .15S            ;BR=BYTE NOT EMPTY YET
        LDB     ,U+             ;NEXT COMPRESSED BYTE
        ASLB                    ;NEXT COMPRESSED BIT
        INCB                    ;SET BYTE EMPTY FLAG
.15S    BCC     .10S            ; BR=NOT AT END OF LENGTH                       ;;Fixme was: 15$
        ...
        BNE     .30S            ; BR=1 OR MORE RUN LENGTH (A VALID RUN LENGTH)
...
;*
.25S    PULS    D,X,Y,PC        ;RETURN TO CALLER                               ;;Fixme was: 25$
;*
.30S    ANDA    #$07            ;UNCOMAPCT 3 BIT COLOR NIBBLE                   ;;Fixme was: 30$
        BEQ     .33S
        ADDA    #8-1
.33S    EQU     *                                                               ;;Fixme was: 33$
;*
...
WRPIX   PSHS    D,Y             ;SAVE REGGIES AGAIN
        ...
        STA     DRRUC           ;THIS 1ST BECAUSE OF INTERUPTS USING THIS BIT
        STA     RRUC
.33S    TFR     X,D             ;CALC X BYTE LOCATION                           ;;Fixme was: 33$
        RORA
        RORB
        TFR     B,A
        ...
        BCS     .10S            ; BR=LOWER NIBBLE (ODD X PIXEL ADDRESS)
        ASLA
        ASLA
        ASLA
        ASLA
.10S    ORA     ,Y              ;ASSUMES A BLANK SCREEN                         ;;Fixme was: 10$
        STA     ,Y
...
.10S    ASLB                    ;GET NEXT BIT                                   ;;Fixme was: 10$
        BNE     .15S            ;BR=BYTE NOT EMPTY YET
        LDB     ,U+             ;NEXT COMPRESSED BYTE
        ASLB                    ;NEXT COMPRESSED BIT
        INCB                    ;SET BYTE EMPTY FLAG
.15S    ROLA                    ;PUT BIT INTO ANSWER                            ;;Fixme was: 15$
...
SNBUB   FCB     001,$72,1               ;THE SOUND OF A BUBBLE
    ...
    ENDIF
        ...
        ORG     $EFF0
        FDB     SystemVectors,SystemVectors,SystemVectors,SystemVectors,IRQ,SystemVectors,SystemVectors,SystemVectors
