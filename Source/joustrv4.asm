...
;*
        ORG     $0
WBOUND                  ;NBR OF BOUNDERS IN THIS WAVE (UPPER NIBBLE)
WHUNT   RMB     1       ;NBR OF HUNTERS IN THIS WAVE (LOWER NIBBLE)
WLORD                   ;NBR OF SHADOW LORDS IN THIS WAVE (UPPER NIBBLE)
...
.4S     LDX     PWAVE,U         ;GET WAVE AREA TO WORK FROM
        ...
        ANDA    #$0F
        STA     WSMART
.44S    LDB     WSTATUS,X               ;CALL WAVE STARTER ROUTINE
...
.2S     LDA     [PWAVE,U]       ;INTRODUCE BOUNDARS
        ANDA    #$0F
        BEQ     .3S             ;BR=NO BOUNDARS
...
LOCCLR  LDB     WCLENY,Y                ;CALC LOWEST POINT
        ...
        EORB    #4              ;the falling debris                             ;;Fixme was: EORB  #!WDMAFIX
        ADDB    #10+1
        EORB    #4                                                              ;;Fixme was: EORB  #!WDMAFIX
        STD     WCLEN,X
        RTS
...
GENDMA  PSHS    CC
        ...
        STD     DDMA
        LDD     ,X++
        EORA    #$04                                                            ;;Fixme was: EORA  #!HDMAFIX
        EORB    #$04                                                            ;;Fixme was: EORB  #!HDMAFIX
        STD     WDMA
        STX     SDMA
        TFR     Y,D
        STB     KDMA
...
TREFF2  PCNAP   1               ;EFFECTS TIME
        ...
        RORB
        STB     WCX,X
        LDA     #(18/2)!4               ;PROPER X LENGTH FOR BOX ERASE                  ;;Fixme was: LDA  #18/2!X(!WDMAFIX) (Should be $0D - $8C29)
        STA     WCLENX,X
        DEC     PFRAME,U
        LBNE    TREFF
;*
...
ENDAD1  EQU     *       ;CURRENT END ADDRESS
    ...
    IF (* - GameRamEnd) > 0
        ERROR "\a The module is too large to fit in the current address space.  [ GameRam: $\{GameRam} - $\{GameRamEnd} = $\{GameRamEnd - GameRam} vs. actual of $\{* - GameRam} ($\{* - GameRamEnd} bytes too large) ]"
    ENDIF
    IF (* - GameRamEnd) = 0
        MESSAGE "\a The module is precisely sized to the current address space.  [ GameRam: $\{GameRam} - $\{GameRamEnd} = $\{GameRamEnd - GameRam} vs. actual of $\{* - GameRam} ]"
    ENDIF
    IF (* - GameRamEnd) < 0
        WARNING "\a The module is smaller than the current address space.  [ GameRam: $\{GameRam} - $\{GameRamEnd} = $\{GameRamEnd - GameRam} vs. actual of $\{* - GameRam} ($\{GameRamEnd - *} bytes unused) ]"
    ENDIF
...
;??????
        ...
        CMPB    #7!4            ;GUYS LENGTH IS ASSUMED TO BE 7 PIXELS TALL     ;;Fixme was: CMPB  #7!X(!WDMAFIX)
